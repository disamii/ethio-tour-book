    ...
    //   print_r( $tour_package_bd);
      ...
      if(isset($_POST['delete']))
      {
                $delete= $_POST['delete_id'];
                  $executee= "DELETE FROM travel_package  WHERE 
                  package_id ={$delete} ";
                  if(mysqli_query($conn,$executee))
                  {
                    header('location:Admin tour package edit.php' );
...
<html>
  ...
  <a href="Admin tour.php" class="buttonn">Go TO Admin Tour</a>
    ...
    <h1>  <?php echo  $count;?> List Of Packages Are Availble</h1>
    <table>
      <thead>
        <tr>
          <th>Package ID</th>
      ...
      <?php foreach ( $tour_package_bd as $inner_array) :?>
        <tr>
          <td><?php echo $inner_array['package_id']; ?></td>
          <td>  <?php echo $inner_array['package_name']; ?></td>
          <td>
          <?php echo $inner_array['intial_point']; ?>
        </td>
          <td>
          <?php echo $inner_array['dest_point']; ?>
          ...
          <td>
          <?php echo $inner_array['numbers _of_days']; ?>
          </td>
          <td>
          <?php echo $inner_array['intial_date']; ?>
        ...
        //    echo $inner_array['desrciption']; 
          ...
          <td>
          <?php echo $inner_array['image_path'];?>
          </td>
          <td>
            <button class="button"><span class="edit-sign">&#9998;</span>Edit</button></td>
          <td>  
            <Form method="POST" action="<?php $_SERVER['PHP_SELF'] ?>">
            <input type="hidden" name="delete_id" value=" <?php echo $inner_array['package_id']; ?>">

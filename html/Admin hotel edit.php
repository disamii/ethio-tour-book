...
$conn = mysqli_connect($HOSTNAME,$USERNAME,$PASSWORD,$DATABaSE);
      ...
      $excute_tour_sql="select *from  hotel";
    ...
    //   print_r( $tour_package_bd);
      ...
      if(isset($_POST['delete']))
      {
                $delete= $_POST['delete_id'];
                  $executee= "DELETE FROM hotel  WHERE 
                  hotel_id ={$delete} ";
                  if(mysqli_query($conn,$executee))
                  {
                    header('location:Admin hotel edit.php' );
                  }
                  else{
                    die('error');
                  }
...
<html>
  ...
  <body>
    <h1>  <?php echo  $count;?> List Of hotel Are Availble</h1>
    ...
    <table>
      ...
      <?php foreach ( $tour_package_bd as $inner_array) :?>
        <tr>
          <td>
            <?php echo $inner_array['hotel_id']; ?>
          </td>
          <td>  
            <?php echo $inner_array['hotel_name']; ?>
          </td>
          <td>
          ...
          <td>
          <?php echo $inner_array['availbe_for']; ?>
          </td>
          <td>
          <?php echo $inner_array['no_of nights']; ?>
          </td>
          <td>
        ...
        //    echo $inner_array['desrciption']; 
          ...
          <td>  
            <Form method="POST" action="<?php $_SERVER['PHP_SELF'] ?>">
                     <input type="hidden" name="delete_id" value=" <?php echo $inner_array['hotel_id']; ?>"> 
